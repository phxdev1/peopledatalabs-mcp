#!/usr/bin/env pwsh
# Script to add the People Data Labs MCP server to VS Code Roo-Cline extension configuration
...
# VS Code Roo-Cline config file path
$configPath = Join-Path $env:APPDATA "Code\User\globalStorage\rooveterinaryinc.roo-cline\settings\mcp_settings.json"

# Check if the config file exists
if (-not (Test-Path $configPath)) {
    Write-Host "VS Code Roo-Cline configuration file not found at: $configPath"
...
Write-Host "People Data Labs MCP server has been added to VS Code Roo-Cline configuration."
Write-Host "Configuration file: $configPath"
Write-Host "Restart VS Code for the changes to take effect."